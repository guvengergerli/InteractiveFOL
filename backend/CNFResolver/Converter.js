...
function processCNF (cnf) {
  ...
  const mixedClauses = Array(cnf.getClauses()).map(
    clause => new MixedClause(clause)
  )
...
// Process CNF formula and pass to python for resolution
function resolveCNF (cnf) {
  const processed = processCNF(cnf)

  // Pass data to Python
  axios.post('/', {
      flatHornClauses: processed.flatHornClauses,
      flatSatClauses: processed.flatSatClauses
    },)
    .then(function (response) {
      return // code to run upon getting response from server
    })
    .catch(function (error) {
      return // code to run upon server responding with error or request failing
    })
...
function valuationEncoder (valuation, resolvedOrs) {
  ...
  return Object.keys(valuation)
    .map(key => `${key.getPositiveDisjuction()}<=>${valuation[key]}`)
    .join(', ')
}
...
const cnf = new CnfFormula(
  new Clause([
    new Predicate('p', ['X'], false),
    new Predicate('q', ['X'], true)
  ])
)
